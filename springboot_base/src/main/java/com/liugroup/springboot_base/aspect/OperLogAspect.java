package com.liugroup.springboot_base.aspect;
...
public class OperLogAspect {
     ...
     */
    @Pointcut("@annotation(com.liugroup.springboot_base.annotation.OperLog)")
     ...
     */
    @Pointcut("execution(* com.liugroup.springboot_base.controller..*.*(..))")
    ...
    public void saveOperLog(JoinPoint joinPoint, Object keys) {
        ...
        try {
            String operId = UUID.randomUUID().toString().replace("-", "");
            operlog.setOperId(operId); // 主键ID
            ...
            operlog.setOperUserId("zhangsan"); // 请求用户ID
            operlog.setOperUserName("张三"); // 请求用户名称
            operlog.setOperIp("0.0.0.0"); // 请求IP
    ...
    public void saveExceptionLog(JoinPoint joinPoint, Throwable e) {
        ...
        try {
            ...
            // 获取切入点所在的方法
            Method method = signature.getMethod();
            String excId = UUID.randomUUID().toString().replace("-", "");
            excepLog.setExcId(excId);
            // 获取请求的类名
            String className = joinPoint.getTarget().getClass().getName();
            // 获取请求的方法名
            String methodName = method.getName();
            ...
            excepLog.setExcUserId("lisi"); // 操作员ID
            excepLog.setExcUserName("李四"); // 操作员名称
            excepLog.setExcUri(request.getRequestURI()); // 操作URI
            excepLog.setExcIp("127.0.0.1"); // 操作员IP
